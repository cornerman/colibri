...
package object zio extends ZioLowPrio {
  implicit def zioTaskRunEffectRuntime[Env](implicit runtime: Runtime[Env]): RunEffect[RIO[Env, *]] = new RunEffectZIOWithRuntime[Env](runtime)
...
private class SinkZIOWithRuntime[Env](runtime: Runtime[Env]) extends Sink[zio.RSink[Env, *]] {
    def unsafeOnNext[A](sink: zio.RSink[Env, A])(value: A): Unit =
      runtime.unsafeRun(UStream(value).run(sink))

    def unsafeOnError[A](sink: zio.RSink[Env, A])(error: Throwable): Unit =
      runtime.unsafeRun(ZStream.fail(error).run(sink))
}

private class SourceZIOWithRuntime[Env](runtime: Runtime[Env]) extends Source[zio.RStream[Env, *]] {
    def unsafeSubscribe[A](source: zio.RStream[Env, A])(sink: Observer[A]): Cancelable = {
...
private class RunEffectZIOWithRuntime[Env](runtime: Runtime[Env]) extends RunEffect[RIO[Env, *]] {
  override def unsafeRunAsyncCancelable[T](effect: RIO[Env, T], cb: Either[Throwable, T] => Unit): Cancelable = unsafeRunSyncOrAsyncCancelable(RIO.yieldNow *> effect, cb)
