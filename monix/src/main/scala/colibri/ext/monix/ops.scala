...
object ops {
  ...
  implicit class RichProSubject[I, O](val self: MonixProSubject[I, O]) extends AnyVal {
    ...
    def mapProSubject[I2, O2](write: I2 => I)(read: O => O2): MonixProSubject[I2, O2] =
      MonixProSubject.from(self.contramap(write), self.map(read))
    ...
    def collectProSubject[I2, O2](write: PartialFunction[I2, I])(read: PartialFunction[O, O2]): MonixProSubject[I2, O2] =
      MonixProSubject.from(self.redirectCollect(write), self.collect(read))
    ...
    def filterProSubject(write: I => Boolean)(read: O => Boolean): MonixProSubject[I, O] =
      MonixProSubject.from(self.redirectFilter(write), self.filter(read))

    def transformObservable[O2](f: Observable[O] => Observable[O2]): MonixProSubject[I, O2]                        = MonixProSubject.from(self, f(self))
    def transformObserver[I2](f: Observable[I2] => Observable[I]): MonixProSubject[I2, O] with ReactiveConnectable =
      MonixProSubject.connectable(self.redirect(f), self)
    def transformProSubject[I2, O2](write: Observable[I2] => Observable[I])(
        read: Observable[O] => Observable[O2],
    ): MonixProSubject[I2, O2] with ReactiveConnectable                                                            = MonixProSubject.connectable(self.redirect(write), read(self))
  }
  ...
  implicit class RichSubject[T](val self: MonixSubject[T]) extends AnyVal {
    ...
    def collectSubject[T2](write: PartialFunction[T2, T])(read: PartialFunction[T, T2]): MonixSubject[T2] =
      MonixProSubject.from(self.redirectCollect(write), self.collect(read))
    def filterSubject(write: T => Boolean)(read: T => Boolean): MonixSubject[T]                           =
      MonixProSubject.from(self.redirectFilter(write), self.filter(read))
    def transformSubject[T2](write: Observable[T2] => Observable[T])(
        read: Observable[T] => Observable[T2],
    ): MonixSubject[T2] with ReactiveConnectable                                                          = MonixProSubject.connectable(self.redirect(write), read(self))
  }
}
