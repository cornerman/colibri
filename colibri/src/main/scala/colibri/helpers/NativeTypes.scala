...
object NativeTypes {
  ...
  val (setImmediateRef, clearImmediateRef): (SetImmediate, ClearImmediate) = {
    if (js.typeOf(js.Dynamic.global.setImmediate) != "undefined")
      (
        js.Dynamic.global.setImmediate.bind(globalObject).asInstanceOf[SetImmediate],
        js.Dynamic.global.clearImmediate.bind(globalObject).asInstanceOf[ClearImmediate],
      )
    else
      (
        js.Dynamic.global.setTimeout.bind(globalObject).asInstanceOf[SetImmediate],
        js.Dynamic.global.clearTimeout.bind(globalObject).asInstanceOf[ClearImmediate],
      )
  }
